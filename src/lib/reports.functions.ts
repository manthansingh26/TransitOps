...
export const getDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [vAll, dAll, tAll, tripsRecent] = await Promise.all([
      s.from("vehicles").select("id,status,type,region"),
      s.from("drivers").select("id,status"),
      s.from("trips").select("id,status,created_at,revenue,fuel_consumed_liters,actual_distance_km"),
      s.from("trips").select("created_at,status").gte("created_at", new Date(Date.now() - 30 * 86400000).toISOString()),
    ]);
    const vehicles = vAll.data ?? [];
    const drivers = dAll.data ?? [];
    const trips = tAll.data ?? [];
...
export const getReports = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [vehicles, trips, fuel, maint, expenses] = await Promise.all([
      s.from("vehicles").select("*"),
      s.from("trips").select("*"),
      s.from("fuel_logs").select("*"),
      s.from("maintenance_logs").select("*"),
    ...
    const perVehicle = vs.map((v) => {
      ...
      const roi = Number(v.acquisition_cost) > 0 ? (revenue - (fuelCost + maintCost)) / Number(v.acquisition_cost) : null;
      return {
        vehicle_id: v.id,
